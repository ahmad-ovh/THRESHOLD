...
def main():
    ...
    if body:
        ...
        # Center body at origin (Feet at Z=0)
        ...
        body.location.x -= (min_x + max_x) / 2.0
        body.location.y -= (min_y + max_y) / 2.0
        body.location.z -= min_z
        bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)

    # 2. Import Head (NO HAIR!)
    ...
    if head:
        ...
        # Approximate Head location on top of neck Z=1.45
        head.location = (-center_x, -center_y, 1.45 - min_z)
        bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)

    # Deselect all
    bpy.ops.object.select_all(action='DESELECT')
    
    # Save .blend file with SEPARATE objects (Body_Mesh and Head_Mesh)
    bpy.ops.wm.save_as_mainfile(filepath=OUTPUT_BLEND)
    print(f"Saved separate Body + Head Blend file: {OUTPUT_BLEND}")

    # Export GLB and OBJ with separate objects intact
    for o in [body, head]:
        if o:
            o.select_set(True)
    ...
    print("Exported Body + Head GLB & OBJ!")
